<template>
  <div
    ...
    :class="[productsInCart.length ? 'bg-cl-secondary' : '']"
  ...
  >
    ...
    <div class="row middle-xs top-sm px40 actions">
      <div class="col-xs-12 col-sm">
        <h2
          v-if="productsInCart.length"
          class="cl-accent shoppingtitle"
        >
          {{ $t('Shopping cart') }}
        </h2>
      </div>
      <div class="col-xs-12 col-sm mt0 end-sm clearcart-col">
        <clear-cart-button
          v-if="productsInCart.length"
    ...
    <div v-if="!productsInCart.length" class="ml30" @click="closeMicrocartExtend">
      There is currently nothing in your shopping cart, browse our range
      <router-link :to="localizedRoute('/')">
       here
      </router-link>
     
    </div>
    <ul v-if="productsInCart.length" class="m0 px40 pb40 products">
    ...
    <div v-if="productsInCart.length" class="summary px40 cl-accent serif">
      <h3 class="m0 weight-400 summary-heading">
        {{ $t('Shopping summary') }}
      </h3>
      
      <div v-for="(segment, index) in filterOutTax()" :key="index" class="row py20">
...
export default {
  ...
  methods: {
    ...
    filterOutTax () {
      ...
      return this.totals.filter(p => (p.code !== 'tax') && (p.code !== 'grand_total') && (p.code !== 'initial_fee') && (p.code !== 'shipping'))
    },
    addDiscountCoupon () {
